...
### 로컬 저장 안정성
- `recipients.json` 저장을 프로세스 전역 잠금과 같은 폴더의 임시 파일로 통일했다. 쓰기를 끝낸 뒤 flush·fsync하고 핸들을 닫은 다음 교체하며, Windows 파일 잠금은 짧은 횟수만 재시도한다.
- 저장에 실패하면 기존 JSON을 빈 파일로 덮어쓰지 않고, 어떤 데이터인지·데이터 폴더·보존 여부·재시도 가능 여부를 로그와 안내창에 표시한다.
- 시작 시 폴더 생성, 파일 교체, SQLite 쓰기, WAL/SHM 생성을 확인한다. 실행 폴더에 안전하게 쓸 수 없으면 `%LOCALAPPDATA%\MAIL_MONSTER_PRO`로 한 번만 옮기고, 원본은 삭제하지 않으며 대상에 이미 있는 파일은 덮어쓰지 않는다.
- 복사된 DB의 읽기 전용 속성을 지우고, 쓰기 실패 시 발송 기록을 초기화하거나 빈 DB로 바꾸지 않는다.

## [v2.8.3] - 2026-09-22

### 다계정 화면
- 시작 시 공급자별 슬롯 화면을 전부 만들지 않는다. 우측 계정 설정·수신처·메시지 발송 화면은 한 세트만 만들고, 계정을 바꿀 때 데이터만 다시 바인딩한다.
- SMTP 계정 수와 계정별 수신자 수에 고정 상한을 두지 않는다. 페이지 크기는 화면 표시 단위이며 저장·대기열·발송 수량을 자르지 않는다.
- 수신자는 선택한 계정의 현재 페이지만 조회한다. Excel 파싱과 DB 저장은 백그라운드에서 처리하고, 다른 계정으로 옮긴 뒤 끝난 결과는 현재 화면에 덮어쓰지 않는다.

### 마지막 템플릿
- 템플릿에 안정적인 ID를 부여하고, 로그인 사용자와 SMTP 계정별로 마지막 선택 ID를 저장한다.
- 프로그램을 다시 실행하면 해당 계정의 제목·본문·첨부·CID를 복원한다. 삭제된 템플릿은 오류 없이 선택을 해제한다.
