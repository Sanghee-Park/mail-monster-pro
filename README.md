...
- **계정별 독립 캠페인**: 각 SMTP 계정(`task_key`)은 자기 수신자·job·queue·runner·상태를 가집니다. 서로 다른 계정은 동시에 실행할 수 있고, 한 계정의 중지·취소·인증 실패가 다른 계정에 영향을 주지 않습니다.
- **중복 차단**: 같은 로그인 사용자·정규화 이메일·최종 본문 HTML 해시 조합은 SMTP 계정이 달라도 DB reservation으로 한 번만 발송합니다.
