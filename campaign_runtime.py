...
def evaluate_send_gate(
    *,
    ...
    if require_running and (job or {}).get("status") != JOB_RUNNING:
        return "not_running"
...
class CampaignRunner:
    ...
    def _pause_scheduled(self, job_id: str) -> str:
        ...
        self.on_progress(self.store.stats_dict(job_id))
        return JOB_SCHEDULED_PAUSE
    ...
    def _halt(self, job_id: str, reason: str) -> str:
    ...
    def wait_for_send_window(self, job_id: str) -> str:
        ...
        while True:
            ...
            job = self.store.get_job(job_id) or {}
            st = job.get("status")
            if st in (JOB_USER_STOPPED, JOB_CANCELLED, JOB_COMPLETED, JOB_NEEDS_ATTENTION):
                return st
    ...
    def _check_attachments(self, job_id: str) -> Optional[str]:
        ...
        if missing:
            return self._halt(job_id, format_missing_files_reason(missing))
        return None
    ...
    def process_one(self, job_id: str, item: dict) -> Optional[str]:
        ...
        if reason == "not_running":
            return job_get_status_safe(self.store, job_id)
        ...
        if kind == "halt":
            ...
            return self._halt(job_id, str(payload or "사용자 확인이 필요합니다."))
        if kind == "error":
    ...
    def run(self, job_id: str, *, wait_off_hours: bool = True) -> str:
        ...
        if job.get("status") in (JOB_USER_STOPPED, JOB_CANCELLED, JOB_COMPLETED):
            return job["status"]
        if self.store.has_live_lease(job_id, now=self.now(), owner=self.owner):
            self.on_log("다른 실행 인스턴스가 이미 이 작업을 발송 중입니다.")
            return "locked"
        ...
        try:
            while True:
                ...
                item = self.store.claim_next_pending(job_id)
                if item is None:
...
def job_get_status_safe(store: CampaignStore, job_id: str) -> str:
